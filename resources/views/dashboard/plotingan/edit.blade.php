...
<div class="h-full ml-14 mt-14 mb-10 md:ml-64">
    <div class="container m-10 p-6 mx-auto">
        <div class="m-5 p-5 ">
            ...
            <form action="{{ route('plotingan.update', $plotingan->id) }}" method="post">
                @method('PUT')
                @csrf
                ...
                <div class="flex justify-center flex-wrap">
                    ...
                    <input type="text" class="hidden" name="kesediaan_id" value="{{ $plotingan->kesediaan->id }}" >
                    <p class=" w-4/5 p-2 m-2">{{ $plotingan->kesediaan->dudi->nama }}</p>
                    <select name="user_id_guru" id="" class="w-4/5 p-2 m-2  text-purple-500 border-b-2 bg-transparent">
                        <option value="">Pilih</option>
                    
                        @foreach ($gurus as $guru)
                        @if($guru->status == 0)
                        <option value="{{ $guru->user_id_guru }}" {{ old('user_id_guru', $plotingan->user_id_guru) == $guru->user_id_guru? 'selected' : null  }}>{{ $guru->user->nama }}</option>
                        @else
                        <option value="{{ $guru->user_id_guru }}" class="hidden">{{ $guru->user->nama }}</option>
                        @endif
                    @endforeach
                    </select>
                @foreach ($plotingan->kesediaan->jurusans as $jurusan)
